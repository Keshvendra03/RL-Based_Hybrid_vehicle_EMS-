...
# FIX 4: n-step returns
from src.agents.nstep_sac import NStepSAC, NStepReplayBuffer

from src.env.ems_env_lookahead import EMSEnv, SOC_TARGET, TERM_TOL
...
def prefill_buffer_benchmark(model: SAC, env: EMSEnv, cycle: str,
    ...
    for _ in range(n_episodes):
        obs, _ = env.reset()
        # resolve the ACTIVE inner env (MultiCycle wrapper switches it each reset)
        inner = getattr(env, "env", env)
        active_cycle = getattr(inner, "cycle_name", cycle)
        ctrl = AdvancedController(active_cycle)
        ctrl.reset()
        while True:
            d = inner._demand
            u = ctrl.step(d["w_MGB"], d["dw_MGB"], d["T_MGB"],
                          d["gear"], inner._Q_BT, d["v"])["u"]
            a = 2.0 * (u - _U_MIN) / (_U_MAX - _U_MIN) - 1.0
...
def rollout_deterministic(model: SAC, cycle: str, eq_factor: float,
                           soc_deadband: float, lookahead: int = 5) -> dict:
...
class EvalAndCheckpoint(BaseCallback):
    def __init__(self, cycle, every_steps: int, out_dir: Path,
                 eq_factor: float, soc_deadband: float, lookahead: int = 5,
                 verbose: int = 1):
        super().__init__(verbose)
        # cycle may be a single name or a list (multi-cycle). Model selection uses
        # the MEAN score across all eval cycles -> picks the best CROSS-CYCLE policy.
    ...
    def _on_step(self) -> bool:
        if self.num_timesteps % self.every == 0:
            ...
            if self.verbose:
                ...
                print(f"[eval @ {self.num_timesteps:>7}] {parts}  "
...
def main():
    ...
    p.add_argument("--lookahead",    type=int,   default=5,
                   help="LOOKAHEAD: upcoming prescribed speeds appended to obs "
                        "(validated to carry engine-off-timing info). 5 = safe; "
                        "0 disables. Replaces the absolute progress fingerprint.")
    p.add_argument("--cycles",       default=None,
                   help="MULTI-CYCLE: comma-separated cycles to interleave per "
                        "episode, e.g. NEDC,FTP75. Overrides --cycle. Required for "
                        "cross-cycle generalization (single-cycle overfits).")
    p.add_argument("--n-step",       type=int,   default=5,
                   help="FIX 4: n-step returns horizon for the critic target. "
                        "1 == stock single-step SAC. 5 (default) injects 5 steps "
                        "of real reward to sharpen long-horizon credit assignment "
                        "(the assist-vs-engine-off decision). Try 3 or 10 to tune.")
    ...
    if len(cycle_list) > 1:
        # Round-robin the training cycle each reset so the agent sees both rhythms
        # WHILE learning (prevents catastrophic forgetting / single-cycle overfit).
    ...
    if args.resume and (out_dir / "sac_ems_last.zip").exists():
        _cls = NStepSAC if args.n_step > 1 else SAC
        model = _cls.load(out_dir / "sac_ems_last", env=env)
        buf = out_dir / "replay_buffer.pkl"
        if buf.exists():
            model.load_replay_buffer(buf)
            # ensure the loaded buffer is the n-step variant
            if args.n_step > 1 and not isinstance(model.replay_buffer, NStepReplayBuffer):
                old = model.replay_buffer
                nb = NStepReplayBuffer(
                    buffer_size=old.buffer_size, observation_space=env.observation_space,
                    action_space=env.action_space, device=model.device, n_envs=1,
                    n_step=args.n_step, gamma=0.9999, handle_timeout_termination=True)
                # copy stored transitions
                nb.observations=old.observations; nb.next_observations=old.next_observations
                nb.actions=old.actions; nb.rewards=old.rewards; nb.dones=old.dones
                nb.timeouts=old.timeouts; nb.pos=old.pos; nb.full=old.full
                model.replay_buffer=nb
        print(f"[resume] loaded sac_ems_last  "
              f"(buffer: {model.replay_buffer.size()} transitions)")
    else:
        model_cls = NStepSAC if args.n_step > 1 else SAC
        model = model_cls(
            "MlpPolicy",
            env,
            learning_rate    = 3e-4,
        ...
        # FIX 4: replace the replay buffer with the n-step buffer (same data
        # layout, so prefill + add() are unchanged; only sampling differs).
        if args.n_step > 1:
            ...
            print(f"[train] FIX 4 active: n-step returns, n={args.n_step}")
