...
class EMSEnv(gym.Env):
    ...
    def __init__(
        ...
        eq_factor: float = 1.0,    # Equivalence factor for battery energy in the
                                   ...
                                   # policies — removed.)
        lookahead: int = 5,        # LOOKAHEAD: number of upcoming prescribed speeds
                                   # appended to the observation. Validated to carry
                                   # real engine-off-timing info the snapshot lacks.
                                   # 5 is the generalization-safe window (long enough
                                   # to inform the energy decision, short enough not to
                                   # fingerprint the cycle). 0 disables (= original obs).
    ) -> None:
        super().__init__()
        if fast_interp:
